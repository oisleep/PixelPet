...
class PrettyBubble(QWidget):
    ...
    def popup(self, text: str, anchor_rect: QtCore.QRect, prefer="right"):
        ...
        start = QtCore.QRect(geo_rect); start.setWidth(int(geo_rect.width()*0.9)); start.setHeight(int(geo_rect.height()*0.9))
    ...
    def paintEvent(self, ev):
        ...
        rect = self.rect(); body = QtCore.QRectF(rect)
    ...
    def sizeHint(self) -> QtCore.QSize: return self._size_hint_for(self._label.text())

    def _suggest_geometry(self, anchor_rect: QtCore.QRect, hint_size: QtCore.QSize, prefer: str) -> QtCore.QRect:
        screen = (QGuiApplication.screenAt(anchor_rect.center())
                  or (self.windowHandle().screen() if self.windowHandle() else None)
                  or QGuiApplication.primaryScreen())
        ...
        candidates = {
            "right": QtCore.QRect(anchor_rect.right()+12, anchor_rect.center().y()-h//2, w, h),
            "left":  QtCore.QRect(anchor_rect.left()-w-12, anchor_rect.center().y()-h//2, w, h),
            "above": QtCore.QRect(anchor_rect.center().x()-w//2, anchor_rect.top()-h-12, w, h),
            "below": QtCore.QRect(anchor_rect.center().x()-w//2, anchor_rect.bottom()+12, w, h),
        }
...
class InputBar(QtWidgets.QWidget):
    ...
    def show_at_bottom(self):
        ...
        rect = QtCore.QRect(int(x), int(y), int(w), int(h)).intersected(geo.adjusted(4,4,-4,-4))
        ...
        self._fade.stop(); self._fade.setStartValue(0.0); self._fade.setEndValue(1.0); self._fade.setEasingCurve(QtCore.QEasingCurve.OutCubic); self._fade.start()
        self.edit.setFocus(); self.edit.selectAll()
    ...
    def hide_with_fade(self):
        if not self.isVisible():
            return
        self._disconnect_fade_finished()
        self._fade.stop(); self._fade.setStartValue(self.windowOpacity()); self._fade.setEndValue(0.0); self._fade.setEasingCurve(QtCore.QEasingCurve.InCubic)
