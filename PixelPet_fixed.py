...
本版改动（含稳定性修复，Windows 专项“保险丝”）：
- 单击香蕉：不再变色；改为在桌面底部弹出半透明输入框，提交后香蕉用自己的气泡回复
- 菜单移除“重置成熟度”（保留在菜单构造里可自行删）
- 修复 Windows 多屏/高 DPI 透明窗越界导致的 UpdateLayeredWindowIndirect 报错：
  * 输入条/气泡定位到正确屏幕并进行边界钳制 + 几像素硬裁剪
  * Windows 下彻底禁用图形阴影（NoDropShadowWindowHint + setGraphicsEffect(None)）
  * 在创建 QApplication 前设置高 DPI 四舍五入策略
"""
...
class PrettyBubble(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Windows 下彻底禁用阴影，避免 dirty 区域外扩；其它平台保留柔和阴影
    ...
    def _suggest_geometry(self, anchor_rect: QRect, hint_size: QSize, prefer: str) -> QRect:
        ...
        for k in order:
            ...
            r = QRect(x, y, w, h)
            # 硬裁剪到屏幕内部留 4px 内边距，避免 layered dirty 负坐标
            r = r.intersected(geo.adjusted(4,4,-4,-4))
...
class InputBar(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        # 加 NoDropShadowWindowHint；Windows 下彻底禁用阴影
        super().__init__(parent, Qt.Tool | Qt.FramelessWindowHint | Qt.NoDropShadowWindowHint)
        self.setAttribute(Qt.WA_TranslucentBackground, True)
        self.setWindowFlag(Qt.WindowStaysOnTopHint, True)
    ...
    def show_at_bottom(self):
        ...
        # 边界钳制 + 硬裁剪，避免越界引发 layered window 报错
        pad = 12
...
class PetWindow(QtWidgets.QWidget):
    ...
    def _make_menu(self) -> QtWidgets.QMenu:
        m = QtWidgets.QMenu()
        m.addAction("打开聊天窗…", self.open_chat)   # 可选：独立聊天窗
...
# --------------------------- 入口 ---------------------------
def main():
    # Qt6：避免 125%/150% 缩放下的坐标取整误差（要在 QApplication 之前）
    try:
