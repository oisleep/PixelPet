...
v3 关键修复：
- 解决“输入条第二次弹出立刻消失”：原因是复用同一个 QPropertyAnimation，
  上次 hide_with_fade() 给 finished 连接了 self.hide，下一次“淡入结束”也会触发 hide。
  现在在 show_at_bottom() 里主动断开该连接，仅在 hide 时连接。
- 继续保留 Windows 稳定性：禁用阴影 + 边界钳制 + 4px 硬裁剪。
- 单击香蕉为“召唤并聚焦”行为，不再切换隐藏。
"""
...
class PrettyBubble(QWidget):
    ...
    def popup(self, text: str, anchor_rect: QRect, prefer="right"):
        ...
        start = QRect(geo)
        start.setWidth(int(geo.width() * 0.9))
        start.setHeight(int(geo.height() * 0.9))
    ...
    def _prepare_text(self, text: str):
        ...
        self._label.setFixedWidth(self._max_width)
        self.adjustSize()
...
class InputBar(QtWidgets.QWidget):
    ...
    def _disconnect_fade_finished(self):
        # 断开之前可能连接的 finished->hide，避免淡入结束时也触发 hide
        try:
...
class PetWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.tray = QtWidgets.QSystemTrayIcon(
            QtGui.QIcon.fromTheme("emblem-favorite"), self
        )
    ...
    def auto_bubble(self):
        ...
        if choice < 0.3:
            ...
            def _work():
                ...
                system = (
                    "你是像素香蕉，用中文简短回复（≤3句），不要输出<think>或过程标签。"
                )
