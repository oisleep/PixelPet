...
class InputBar(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.edit.setPlaceholderText("和不拿拿聊点什么…（Enter 发送，Esc 关闭）")
    ...
    def show_at_bottom(self):
        ...
        try:
            self.activateWindow()
            QtWidgets.QApplication.setActiveWindow(self)   # 让 App 进入前台（macOS 尤其需要）
        except Exception:
            pass
        QtCore.QTimer.singleShot(0, lambda: (
            self.edit.setFocus(QtCore.Qt.ShortcutFocusReason),
            self.edit.selectAll()
        ))
