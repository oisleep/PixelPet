...
import json, random, threading, time, difflib, re
...
class PrettyBubble(QWidget):
    ...
    def _suggest_geometry(self, anchor_rect: QRect, hint_size: QSize, prefer: str) -> QRect:
        ...
        elif QGuiApplication.primaryScreen():
            screen = QGuiApplication.primaryScreen().availableGeometry()
        else:
            screen = anchor_rect.adjusted(-400, -400, 400, 400)
        ...
        for k in order:
            r = candidates[k]
            if screen.contains(r.adjusted(-8,-8,8,8)):
                self._tail_side = {"right":"left","left":"right","above":"bottom","below":"top"}[k]
                return r
        self._tail_side = "bottom"
        return QRect(screen.center().x()-w//2, screen.center().y()-h//2, w, h)
...
class PetWindow(QtWidgets.QWidget):
    ...
    def _make_menu(self) -> QtWidgets.QMenu:
        m = QtWidgets.QMenu()
        m.addAction("打开聊天窗…", self.open_chat)   # 仍保留可选的独立聊天窗
