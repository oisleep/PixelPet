...
v4 更新（按你的需求）
1) 去掉所有 random.choice：需要“随机/多样”处改为让本地模型生成；
   若本地模型不可用，则用一个固定的简洁提醒（非随机）。
2) 启动默认出现在“桌面中间”。
3) 所有消息气泡宽度随消息长度动态变化（基于文本像素宽度和屏幕宽度上限自适应）。
4) 更换系统托盘与窗口图标为“像素香蕉”图标（代码里用同款像素网格绘制）。

仍保留：
- Windows 稳定性“保险丝”：禁用阴影 + 边界钳制 + 4px 硬裁剪；
- 输入条淡入淡出动画的复用修复（防止“弹出立刻消失”）。
"""
...
def banana_pixmap(size: int = 32) -> QtGui.QPixmap:
    """绘制一个像素香蕉图标并返回 QPixmap（用于 tray 与窗口图标）"""
    # 与 BananaSprite 使用同样 22x16 的像素网格
    w, h = 22, 16
    # 尺寸缩放：尽量填满 size（保持像素感，不开抗锯齿）
    s = max(1, min(size // w, size // h))  # 单元像素边长
    img_w, img_h = w * s, h * s
...
class PrettyBubble(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(
            parent, Qt.Tool | Qt.FramelessWindowHint | Qt.NoDropShadowWindowHint
        )
        ...
        self._min_width = 140
        self._max_width = 360
        self._last_geo = None  # 最近使用屏幕几何，便于动态最大宽
    ...
    def popup(self, text: str, anchor_rect: QRect, prefer="right"):
        ...
        self._prepare_text(text)  # 准备文本 & 自适应标签宽度
    ...
    def _prepare_text(self, text: str):
        ...
        # 理想宽：等于“单行宽 + 16”，并钳制在 [min_w, max_w]，实现“随长度变宽”
    ...
    def _suggest_geometry(
        self, anchor_rect: QRect, hint_size: QSize, prefer: str
    ) -> QRect:
        screen = (
            QGuiApplication.screenAt(anchor_rect.center())
            or (self.windowHandle().screen() if self.windowHandle() else None)
...
class LocalModelClient:
    ...
    def ask(
        ...
        msg2 = self._post_chat(
            msgs2, {"num_predict": 256, "temperature": 0.6, "stop": SOFT_STOPS}
        )
...
class Bubble(QtWidgets.QWidget):
    def __init__(self, parent: QtWidgets.QWidget, text: str, ms: int = 2800):
        ...
        self.hide_timer.setInterval(ms)
    ...
    def resize_to_text(self):
        ...
        w = min(
            max_w,
            max(self.fm.horizontalAdvance(line) for line in lines) + 2 * self._pad,
        )
    ...
    def paintEvent(self, e: QtGui.QPaintEvent):  # noqa
        ...
        body = QtCore.QRect(
            rect.left(), rect.top(), rect.width(), rect.height() - self._arrow
        )
        ...
        tri = QtGui.QPolygon(
            [
                ...
                QtCore.QPoint(
                    body.left() + body.width() // 3 + 6, body.bottom() + self._arrow
                ),
            ]
        )
        p.setOpacity(self._opacity)
        p.setPen(QtCore.Qt.NoPen)
        p.setBrush(QtGui.QColor(30, 30, 30, 230))
        p.drawPath(path)
        p.drawPolygon(tri)
        p.setPen(QtGui.QColor(240, 240, 240))
        x = body.left() + self._pad
        y = body.top() + self._pad + self.fm.ascent()
...
class InputBar(QtWidgets.QWidget):
    ...
    def show_at_bottom(self):
        ...
        rect = QRect(int(x), int(y), int(w), int(h)).intersected(
...
class PetWindow(QtWidgets.QWidget):
    ...
    def auto_bubble(self):
        ...
        if choice < 0.3:
            ...
            # 替换 random.choice：改为让本地模型直接生成一句话；如果模型不可用，用固定文本（非随机）。
