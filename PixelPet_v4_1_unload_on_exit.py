...
"""
Pixel Banana v4.1
- 退出时卸载模型（keep_alive=0），避免 Ollama 长驻占用显存/内存（服务仍运行但空闲）。
- 所有对 /api/chat 的调用都带 keep_alive=0（一次性调用，用完即卸）。
- 增加设置项：unload_on_exit（默认 True）。
- 其他行为承接 v4：启动居中、气泡宽度自适应、图标为像素香蕉、点击香蕉呼出输入条。
"""
...
DEFAULT_CFG = {
    ...
    "auto_bubble": True,
    "opacity": 0.98,
    "unload_on_exit": True,  # 新增：退出时卸载模型
}
...
def banana_pixmap(size: int = 32) -> QtGui.QPixmap:
    ...
    for y,row in enumerate(body):
        for x,ch in enumerate(row):
            if ch=="1": points.append((x,y,1))
    shadow = [(x,y) for y,row in enumerate(body) for x,ch in enumerate(row) if ch=="1" and (y>=9 or x>=12)]
    for (x,y) in shadow: points.append((x, y+1 if y+1<h else y, 2))
    points += [(18,2,3),(19,2,3),(7,4,4),(8,5,4),(6,6,4)]

    for (x,y,val) in points:
        color = c_body if val==1 else c_shadow if val==2 else c_stalk if val==3 else c_high
...
class PrettyBubble(QWidget):
    ...
    def set_max_width(self, w: int): self._max_width = max(200, int(w))
    ...
    def popup(self, text: str, anchor_rect: QRect, prefer="right"):
        ...
        geo = screen.availableGeometry(); self._last_geo = geo
        ...
        start = QRect(geo_rect); start.setWidth(int(geo_rect.width()*0.9)); start.setHeight(int(geo_rect.height()*0.9))
        ...
        self._pop.stop(); self._pop.setStartValue(start); self._pop.setEndValue(geo_rect); self._pop.start()
    ...
    def _size_hint_for(self, text: str) -> QSize:
        ...
        if self._last_geo: w = min(w, int(self._last_geo.width() * 0.60))
        return QSize(w, h)
...
class Settings:
    ...
    def save(self) -> None:
        data = {"model_url": self.model_url, "model_name": self.model_name, "city": self.city,
                "auto_bubble": self.auto_bubble, "opacity": self.opacity, "unload_on_exit": self.unload_on_exit}
        CONF_DIR.mkdir(parents=True, exist_ok=True)
...
class LocalModelClient:
    ...
    def _post_chat(self, messages, options, keep_alive_sec: int = 0) -> str:
        payload = {"model": self.model_name, "messages": messages, "stream": False,
                   "options": options, "keep_alive": keep_alive_sec}
    ...
    def ask(self, prompt: str, system: Optional[str] = None, no_think: bool = True) -> str:
        ...
        msg1 = self._post_chat(msgs, {"num_predict": 256, "temperature": 0.6}, keep_alive_sec=0)
        clean1 = strip_thinking(msg1)
        if clean1: return clean1

        msgs2 = list(msgs)
        if no_think: msgs2[0] = {"role":"system","content":sys_prompt + " 严禁输出思考或任何标签，仅一句话答案。"}
        msg2 = self._post_chat(msgs2, {"num_predict": 256, "temperature": 0.6, "stop": SOFT_STOPS}, keep_alive_sec=0)
    ...
    def unload(self) -> bool:
        """主动请求卸载当前模型（不会关闭 Ollama 服务器，仅释放显存/内存）。"""
        try:
            payload = {"model": self.model_name, "prompt": "", "stream": False, "keep_alive": 0}
...
class SelfCheckDialog(QtWidgets.QDialog):
    ...
    def _run(self):
        ...
        try:
            ...
            payload = {"model": name, "messages": [{"role":"user","content":"只回：香蕉OK"}], "stream": False, "keep_alive": 0}
...
class InputBar(QtWidgets.QWidget):
    ...
    def show_at_bottom(self):
        ...
        rect = QRect(int(x), int(y), int(w), int(h)).intersected(geo.adjusted(4,4,-4,-4))
        ...
        self._fade.stop(); self._fade.setStartValue(0.0); self._fade.setEndValue(1.0); self._fade.setEasingCurve(QEasingCurve.OutCubic); self._fade.start()
        self.edit.setFocus(); self.edit.selectAll()
    ...
    def hide_with_fade(self):
        if not self.isVisible():
            return
        self._disconnect_fade_finished()
        self._fade.stop(); self._fade.setStartValue(self.windowOpacity()); self._fade.setEndValue(0.0); self._fade.setEasingCurve(QEasingCurve.InCubic)
...
class PetWindow(QtWidgets.QWidget):
    ...
    def __init__(self, app: QtWidgets.QApplication):
        ...
        self.tray.setToolTip("像素香蕉"); self.tray.setIcon(icon); self.tray.setVisible(True)
    ...
    def _make_menu(self) -> QtWidgets.QMenu:
        ...
        act_unload.setCheckable(True); act_unload.setChecked(self.settings.unload_on_exit)
    ...
    def toggle_unload_on_exit(self):
        ...
        self.settings.save()
        self.say("已开启：退出时卸载模型" if self.settings.unload_on_exit else "已关闭：退出时卸载模型")
